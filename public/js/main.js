...
function getVisualArch(key) {
  const L = '/assets/icons/logos';
  const S = '/assets/icons/simple-icons';
  const ico = {
...
const platformDetails = {
  azure: {
    logo: '/assets/icons/logos/microsoft-azure.svg',
  ...
  aws: {
    logo: '/assets/icons/logos/aws.svg',
    title: 'Amazon AWS',
  ...
  gcp: {
    logo: '/assets/icons/logos/google-cloud.svg',
    title: 'Google Cloud',
...
// ── Mobile Nav ──
function openMobile() { document.getElementById('mobileNav').classList.add('active'); document.querySelector('.menu-toggle')?.setAttribute('aria-expanded', 'true'); }
function closeMobile() { document.getElementById('mobileNav').classList.remove('active'); document.querySelector('.menu-toggle')?.setAttribute('aria-expanded', 'false'); }
...
(function() {
  ...
  if (!document.getElementById('chatFab')) {
    var chatHTML = ''
      ...
      +   '<span class="chat-fab-icon"><svg class="bot-avatar-svg" viewBox="0 0 100 100" width="44" height="44"><defs><linearGradient id="botGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a855f7"/><stop offset="50%" stop-color="#7c3aed"/><stop offset="100%" stop-color="#dc2626"/></linearGradient></defs><path d="M15 55 C15 30 30 15 50 15 C70 15 85 30 85 55 C85 72 74 82 65 85 L55 95 L50 85 C30 85 15 72 15 55Z" fill="url(#botGrad)"/><circle cx="37" cy="52" r="5" fill="#fff"/><circle cx="63" cy="52" r="5" fill="#fff"/><circle cx="37" cy="53" r="2.5" fill="#1e1b4b"/><circle cx="63" cy="53" r="2.5" fill="#1e1b4b"/><path d="M42 66 Q50 73 58 66" stroke="#fff" stroke-width="2.5" fill="none" stroke-linecap="round"/><line x1="62" y1="22" x2="70" y2="8" stroke="url(#botGrad)" stroke-width="3" stroke-linecap="round"/><circle cx="72" cy="6" r="5" fill="#3b82f6"/></svg></span>'
      +   '<span class="chat-fab-close">✕</span>'
      + '</button>'
      + '<div class="chat-window" id="chatWindow">'
      +   '<div class="chat-header">'
      +     '<span class="chat-header-avatar"><svg class="bot-avatar-svg" viewBox="0 0 100 100" width="36" height="36"><defs><linearGradient id="botGradH" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a855f7"/><stop offset="50%" stop-color="#7c3aed"/><stop offset="100%" stop-color="#dc2626"/></linearGradient></defs><path d="M15 55 C15 30 30 15 50 15 C70 15 85 30 85 55 C85 72 74 82 65 85 L55 95 L50 85 C30 85 15 72 15 55Z" fill="url(#botGradH)"/><circle cx="37" cy="52" r="5" fill="#fff"/><circle cx="63" cy="52" r="5" fill="#fff"/><circle cx="37" cy="53" r="2.5" fill="#1e1b4b"/><circle cx="63" cy="53" r="2.5" fill="#1e1b4b"/><path d="M42 66 Q50 73 58 66" stroke="#fff" stroke-width="2.5" fill="none" stroke-linecap="round"/><line x1="62" y1="22" x2="70" y2="8" stroke="url(#botGradH)" stroke-width="3" stroke-linecap="round"/><circle cx="72" cy="6" r="5" fill="#3b82f6"/></svg></span>'
...
(function() {
  var GTM_ID = 'GTM-XXXXXXX'; // TODO: Replace with your Google Tag Manager ID
  if (GTM_ID === 'GTM-XXXXXXX') return; // Skip if not configured
  // GTM head script
...
// ── SEO Event Tracking (dataLayer for GTM) ──
window.dataLayer = window.dataLayer || [];

// Track CTA button clicks
document.querySelectorAll('.btn-glow, .btn-glass').forEach(function(btn) {
  ...
  window.addEventListener('scroll', function() {
    ...
    milestones.forEach(function(m) {
      if (scrollPct >= m && !triggered[m]) {
        triggered[m] = true;
        window.dataLayer.push({ event: 'scroll_depth', depth: m + '%', page_path: window.location.pathname });
      }
    });
  });
})();
...
(function() {
  [30, 60, 120].forEach(function(sec) {
    setTimeout(function() {
      window.dataLayer.push({ event: 'time_on_page', seconds: sec, page_path: window.location.pathname });
    }, sec * 1000);
  });
})();
...
document.querySelectorAll('a[href^="http"]').forEach(function(link) {
  if (link.hostname !== window.location.hostname) {
    link.addEventListener('click', function() {
      window.dataLayer.push({ event: 'outbound_click', url: link.href, page_path: window.location.pathname });
    });
  }
});

// UTM parameter capture — stores in sessionStorage for contact form
(function() {
